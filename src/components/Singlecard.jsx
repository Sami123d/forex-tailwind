...
const Singlecard = ({ title, price, storage, user, send, image, isSpecial,  }) => {
  ...
  return (
    <div className={`${isSpecial ? "flex-1 md:scale-110" : "flex-1"}`}>
      <div className={ ` cursor-default p-6 rounded-lg text-center shadow-md transform transition duration-300 ${isSpecial ? "hover:shadow-2xl bg-[#eceef3]" : "hover:shadow-2xl" }`}>
      ...
      <p className="text-gray-700 py-2 border-b-2 " style={{borderColor:"#d5d7da"}}>{user}</p>
      <p className="text-gray-700 py-2 border-b-2 " style={{borderColor:"#d5d7da"}}>{send}</p>
